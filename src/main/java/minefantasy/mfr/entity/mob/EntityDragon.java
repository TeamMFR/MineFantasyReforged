...
public class EntityDragon extends EntityFlying implements IMob, IArmouredEntity, IArmourPenetrationMob {
    ...
    protected void initEntityAI() {
        this.tasks.addTask(0, new EntityAIDragonRandomFlyMovement(this));
    ...
    private static class EntityAIDragonRandomFlyMovement extends EntityAIBase {

        private EntityDragon dragon;
        private Random random = new Random();
        private int count = random.nextInt(90);
        private int direction = random.nextBoolean() ? 1 : -1;

        public EntityAIDragonRandomFlyMovement(EntityDragon dragon) {
            this.dragon = dragon;
        }
        ...
        public boolean shouldContinueExecuting() {
            if (dragon.getAttackTarget() == null ) {
                dragon.move(MoverType.SELF, dragon.getLookVec().x / 2, ThreadLocalRandom.current().nextDouble(-1, 1), dragon.getLookVec().z / 2);
                if (count != 0) {
                ...
                if(dragon.shouldFollow) {
                    Vec3d vec = new Vec3d(dragon.getAttackTarget().getPosition().subtract(dragon.getPosition()));
                    dragon.move(MoverType.SELF, vec.x, vec.y, vec.z);
                    count = 10;
                } else {
                    if(count != 0){
                        count--;
