...
public class StructureGenDSRoomSml2 extends StructureModuleMFR {
    public StructureGenDSRoomSml2(World world, BlockPos pos, EnumFacing facing, Random random) {
    ...
    private IBlockState getFloor(int width, int depth, int x, int z) {
        ...
        return MineFantasyBlocks.REFINED_PLANKS.getDefaultState();
    }
    ...
    private void buildHomeFurnishings(int width, int depth, int height) {
        ...
        for (int x = width - 1; x >= (width - 4); x--) {
            placeBlockWithState(world, getRandomVariantBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, random), x, 1, 5);
            placeBlockWithState(world, getRandomVariantBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, random), x, 2, 5);
            placeBlock(world, Blocks.STONE_SLAB, x, 3, 5);
        }
        placeBlock(world, Blocks.STONE_SLAB, width - 1, 1, 7);
        placeBlock(world, Blocks.STONE_SLAB, width - 2, 1, 7);
        placeBlock(world, Blocks.STONE_SLAB, width - 1, 1, 8);
        placeBlock(world, Blocks.STONE_SLAB, width - 2, 1, 8);
        ...
        placeChest(world,width - 3, 1, 7, MineFantasyLoot.DWARVEN_HOME_RICH);
    }

    private void placeChest(World world, int x, int y, int z, ResourceLocation loot) {
