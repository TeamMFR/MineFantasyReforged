...
public class StructureDSSurfaceAppendage extends StructureModuleMFR {
	...
	public void generate() {
		...
		for (int x = -width; x <= width; x++) {
			for (int z = 0; z <= depth; z++) {
				...
				placeBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, pos.add(0, -1, 0));
				if (z < depth) {
					placeBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, pos.add(0, -2, 0));
				}
				if (z < depth - 1) {
					placeBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, pos.add(0, -3, 0));
				}
				// WALLS
				for (int y = 0; y <= height; y++) {
	...
	private Object[] getRoofDecor(int width, int depth, int x, int z) {
		if (x == -width || x == width || z == 0 || z == depth) {
			...
			return new Object[] {MineFantasyBlocks.BRONZE_BARS, false};
		}
		return null;
	}
	...
	private void decorateTower(int width, int depth, int height) {
		for (int y = 1; y <= height; y++) {
			if (y >= 2 && y <= (height - 2) && (y < (height / 2) - 1 || y > (height / 2) + 1)) {
				this.placeBlock(MineFantasyBlocks.BRONZE_BARS, new BlockPos(-width, y, depth / 2));
				this.placeBlock(MineFantasyBlocks.BRONZE_BARS, new BlockPos(width, y, depth / 2));
				this.placeBlock(MineFantasyBlocks.BRONZE_BARS, new BlockPos(0, y, depth));
				if (y > (height / 2) + 1) {
					this.placeBlock(MineFantasyBlocks.BRONZE_BARS, new BlockPos(0, y, 0));
				}
			}
	...
	private void buildHomeFurnishings(int width, int depth, int height) {
		for (int x = width - 1; x >= (width - 4); x--) {
			placeBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, new BlockPos(x, 1, 3));
			placeBlock(MineFantasyBlocks.REINFORCED_STONE_BRICKS, new BlockPos(x, 2, 3));
		...
		placeChest(new BlockPos(width - 3, 1, depth - 1), MineFantasyLoot.DWARVEN_HOME);

		for (int x = -(width - 1); x < width; x++) {
			for (int z = 1; z < depth; z++) {
				if (x == width - 1 || x == -(width - 1) || z == 1 || z == (depth - 1)) {
					placeBlock(MineFantasyBlocks.REINFORCED_STONE, new BlockPos(x, height + 1, z));
				}
			}
		}
	}
