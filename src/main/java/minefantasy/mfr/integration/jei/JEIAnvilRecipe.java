...
public class JEIAnvilRecipe implements IRecipeWrapper {
	...
	public JEIAnvilRecipe(AnvilRecipeBase recipe, IStackHelper stackHelper) {
		...
		this.recipe = recipe;
		if (AnvilRecipeBase.isCustomRecipe(recipe)) {
			List<ItemStack> stacks;
			if (recipe.getAnvilRecipeOutput().getItem() instanceof IMaterialComponent) {
				stacks = CustomToolHelper.constructAllVariants(recipe.getAnvilRecipeOutput().getItem());
				...
				if (recipe.isTierModifyOutputCount()) {
					stacks = CustomToolHelper
							.constructModifiedCounts(recipe.getAnvilRecipeOutput(), CustomMaterialType.METAL_MATERIAL);
				}
				else {
					stacks = Collections.singletonList(recipe.getAnvilRecipeOutput());
				}
			}
			results = Collections.singletonList(stacks);
		}
		else {
			result = recipe.getAnvilRecipeOutput();
		}
	...
	public AnvilRecipeBase getRecipe() {
		return recipe;
	}
}
