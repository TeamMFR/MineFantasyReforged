...
public class GuiCookingBench extends GuiContainer {
	private TileEntityCookingBench tile;
	private int regularXSize = 176;

	public GuiCookingBench(ContainerBase container, TileEntityCookingBench tile) {
		super(container);
		this.xSize = 195;
		this.ySize = 240;
		this.tile = tile;
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		...
		String s = MineFantasyReforged.isDebug() ? "Cooking Bench Crafting" : knowsCraft ? I18n.format(tile.getResultName()) : "????";
		...
		if (knowsCraft && !tile.getResultName().equalsIgnoreCase("")) {
			...
			if (x < xPoint + regularXSize + 20 && x > xPoint + regularXSize && y < yPoint + 20 && y > yPoint) {
				//todo make tool icons for cooking bench
				String s2 = I18n.format("tooltype.carpenter") + ", "
						+ (tile.getCookingBenchTierNeeded() > -1
						? I18n.format("attribute.mfcrafttier.name") + " "
						+ tile.getCookingBenchTierNeeded()
						: I18n.format("attribute.nomfcrafttier.name"));
	...
	protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
		...
		//todo: make a new texture for the cooking bench
		...
		if (tile.doesPlayerKnowCraft(mc.player) && !tile.getResultName().equalsIgnoreCase("")) {
			//todo make tool icons for cooking bench
			GuiHelper.renderToolIcon(this, "carpenter", tile.getCookingBenchTierNeeded(), xPoint + regularXSize, yPoint,
					true);
