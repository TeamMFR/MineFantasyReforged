...
public class RandomDigs 
{
	 ...
	 */
	public static void addOre(ItemStack drop, float chance, Block block, int meta, int harvestLevel, int min, int max, boolean silkDisable)
	{
	...
	{
		drops.add(new RandomDigs(drop, chance/100F, block, OreDictionary.WILDCARD_VALUE, harvestLevel, min, max, silkDisable));
	}
	...
	private static boolean matchesOre(RandomDigs ore, Block base, int meta, int harvest, float multiplier, boolean silktouch, int y) 
	{
	...
	public RandomDigs(ItemStack drop, float chance, Block base, int meta, int harvestLevel, int min, int max, boolean silkDisable)
	{
