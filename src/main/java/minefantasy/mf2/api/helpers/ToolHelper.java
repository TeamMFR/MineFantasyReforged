...
public class ToolHelper
{
	...
	/**
	 * Compares an itemstack to see if the item is made from the said material
	 * ONLY APPLIES TO IToolMaterial in api.
	 */
	...
	public static int setDuraOnQuality(ItemStack item, int dura)
	{
		...
		if(quality < 100)
		{
			dura -= ((dura*0.75) / 100 * (100F - quality));//This means 100- takes upto 75% from it's max durability lvl0 is 25%dura
		}
		return dura;
	}
	...
	public static float modifyDigOnQuality(ItemStack item, float digspeed)
	{
		...
		if(quality < 100)
		{
			digspeed -= ((digspeed*0.5) / 100 * (100F - quality));//This means 100- takes upto 50% from it's max speed lvl0 is 50% speed
		}
		return digspeed;
	}
	...
	public static float modifyDamOnQuality(ItemStack item, float damage)
	{
		...
		if(quality < 100)
		{
			damage -= ((damage*0.25) / 100 * (100F - quality));//This means 100- takes upto 25% from it's max damage lvl0 is 75% damage
		}
		return damage;
	}
	...
	public static float modifyArmourRating(ItemStack item, float rating)
	{
		...
		if(quality < 100)
		{
			rating -= ((rating*0.5) / 100 * (100F - quality));//This means 100- takes upto 50% from it's max armour lvl0 is 50% armour
		}
		return rating;
	}
