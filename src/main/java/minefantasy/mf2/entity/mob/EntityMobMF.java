...
public abstract class EntityMobMF extends EntityCreature implements IMob
{
    ...
    public void onUpdate()
    {
        ...
        if (this.canDespawn() && !this.worldObj.isRemote && this.worldObj.difficultySetting == EnumDifficulty.PEACEFUL)
        {
            this.setDead();
        }
    }
    ...
    {
        if (this.attackTime <= 0 && dam < 2.0F && target.boundingBox.maxY > this.boundingBox.minY && target.boundingBox.minY < this.boundingBox.maxY)
        {
            this.attackTime = 20;
