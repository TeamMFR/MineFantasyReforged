...
public class EntityDragon extends EntityFlyingMF implements IMob, IBossDisplayData, IArmouredEntity, IArmourPenetrationMob
{
    ...
    public void onUpdate()
    {
    	...
    	if(!worldObj.isRemote)
    	{
	    	...
	    	if (targetedEntity == null && (rand.nextInt(100) == 0) && disengageTime <= 0)
	    	{
	            ...
	            this.waypointY = this.posY + (double) ((this.rand.nextFloat() * 1.5F - 1.0F) * 12.0F);
        	...
        	{
	        	++interestTime;
	        	if(interestTime > maxTime && worldObj.canBlockSeeTheSky((int)posX, (int)posY, (int)posZ) && posY > 128F)
	        	{
	        		if(ConfigMobs.dragonMSG)
	        		{
		        		...
		        		while(players.hasNext())
		        		{
		        			...
		        			if(instance != null && instance instanceof EntityPlayer)
		        			{
		        				...
		        				{
		        					((EntityPlayer)instance).addChatMessage(new ChatComponentText(StatCollector.translateToLocal("event.dragonaway.name")));
		        				}
		        			}
		        		}
	        		}
    ...
    protected void updateEntityActionState()
    {
        ...
        if (this.targetedEntity != null && getDisengageTime() <= 0 && this.targetedEntity.getDistanceSqToEntity(this) < var9 * var9) 
        {
        	if (!worldObj.isRemote && !this.canEntityBeSeen(this.targetedEntity) && getDisengageTime() <= 0 && (targetedEntity instanceof EntityPlayer ? rand.nextInt(40) == 0 : true))
            {
        		disengage(200);
        		return;
            }
    ...
    public boolean attackEntityFrom(DamageSource source, float damage) 
	{
        ...
        {
            if(getDisengageTime() <= 0 && getAttackTarget() == null || damage > 16 || (targetedEntity != null && !(targetedEntity instanceof EntityPlayer)))
