...
public class EntityArrowMF extends EntityArrow implements IProjectile, IDamageType, IArrowRetrieve
{
	...
	public void setVelocity(double xv, double yv, double zv) 
	{
		...
		if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) 
		{
			...
			this.prevRotationYaw = this.rotationYaw = (float) (Math.atan2(xv,
					zv) * 180.0D / Math.PI);
	...
	public void onUpdate()
	{
		...
		} else
		{
			...
			if (movingobjectposition != null) 
			{
				...
				else 
				{
					...
					f2 = MathHelper.sqrt_double(this.motionX * this.motionX
							...
							* this.motionZ);
					this.posX -= this.motionX / f2
							* 0.05000000074505806D;
					this.posY -= this.motionY / f2
							* 0.05000000074505806D;
					this.posZ -= this.motionZ / f2
							* 0.05000000074505806D;
					playHitSound();
					this.inGround = true;
					this.arrowShake = 7;
			...
			for (this.rotationPitch = (float) (Math.atan2(this.motionY,
					f2) * 180.0D / Math.PI); this.rotationPitch
	...
	public void readEntityFromNBT(NBTTagCompound nbt)
	{
		...
		this.inBlock = Block.getBlockById(nbt
				.getByte("inTile") & 255);
		...
		if (nbt.hasKey("pickup", 99)) {
			...
			this.canBePickedUp = nbt.getBoolean("player") ? 1
					: 0;
		}
	...
	/**
	 * [New Method] Whether the arrow has a stream of critical hit particles flying behind
	 * it. Called by chance
	 */
