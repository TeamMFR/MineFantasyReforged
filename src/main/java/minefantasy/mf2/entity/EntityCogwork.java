...
public class EntityCogwork extends EntityLivingBase implements IPowerArmour
{
	...
	protected void damageEntity(DamageSource source, float dam)
    {
        ...
        {
        	canDestroy = ToolHelper.getCrafterTool(((EntityLivingBase)source.getSourceOfDamage()).getHeldItem()).equalsIgnoreCase("spanner");
        }
        ...
        else if (dam != 0.0F)
        {
            ...
            {
            	this.playSound("mob.irongolem.hit", 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
            }
        }
    }
	...
	{
		return (int)((float)i / getMaxFuel() * getFuel());
	}
    ...
    {
    	Shockwave explosion = new Shockwave("humanstomp", worldObj, this.riddenByEntity != null ? this.riddenByEntity : this, x, y, z, power);
