...
public class EntityParachute extends Entity
{
    ...
    public boolean attackEntityFrom(DamageSource src, float dam)
    {
        ...
        else if (!this.worldObj.isRemote && !this.isDead)
        {
            ...
            boolean flag = src.getEntity() instanceof EntityPlayer && ((EntityPlayer)src.getEntity()).capabilities.isCreativeMode;
    ...
    public void setPositionAndRotation2(double x, double y, double z, float xr, float yr, int zr)
    {
        ...
        this.parachuteY = y;
        this.parachuteZ = z;
        this.parachuteYaw = (double)xr;
        this.parachutePitch = (double)yr;
    ...
    public void onUpdate()
    {
        ...
        if (this.worldObj.isRemote && this.isParachuteEmpty)
        {
            ...
            {
                d2 = this.posX + (this.parachuteX - this.posX) / (double)this.parachutePosRotationIncrements;
                d4 = this.posY + (this.parachuteY - this.posY) / (double)this.parachutePosRotationIncrements;
                d11 = this.posZ + (this.parachuteZ - this.posZ) / (double)this.parachutePosRotationIncrements;
                d12 = MathHelper.wrapAngleTo180_double(this.parachuteYaw - (double)this.rotationYaw);
                this.rotationYaw = (float)((double)this.rotationYaw + d12 / (double)this.parachutePosRotationIncrements);
                this.rotationPitch = (float)((double)this.rotationPitch + (this.parachutePitch - (double)this.rotationPitch) / (double)this.parachutePosRotationIncrements);
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                if (list != null && !list.isEmpty())
                {
                    for (int k1 = 0; k1 < list.size(); ++k1)
                    {
                        ...
                        if (entity != this.riddenByEntity && entity.canBePushed() && entity instanceof EntityParachute)
                        {
