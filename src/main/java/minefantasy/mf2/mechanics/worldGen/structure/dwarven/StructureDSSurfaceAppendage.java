	...
	public void generate()
	{
		...
		for(int x = -width; x <= width; x ++)
		{
			for(int z = 0; z <= depth; z ++)
			{
				...
				placeBlock(BlockListMF.reinforced_stone_bricks, StructureGenAncientForge.getRandomMetadata(rand), x, -1, z);
				if(z < depth)
				{
					placeBlock(BlockListMF.reinforced_stone_bricks, StructureGenAncientForge.getRandomMetadata(rand), x, -2, z);
				}
				if(z < depth-1)
				{
					placeBlock(BlockListMF.reinforced_stone_bricks, StructureGenAncientForge.getRandomMetadata(rand), x, -3, z);
				}
				//WALLS
				for(int y = 0; y <= height; y ++)
				{
