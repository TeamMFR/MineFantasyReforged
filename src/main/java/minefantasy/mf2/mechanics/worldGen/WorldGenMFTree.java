...
public class WorldGenMFTree extends WorldGenAbstractTree
{
    ...
    void func_150530_a(int[] x, int[] y, Block block)
    {
        ...
        if (aint2[b1] != 0)
        {
            ...
            {
                aint3[b1] = MathHelper.floor_double((double)(x[b1] + i) + 0.5D);
                aint3[b2] = MathHelper.floor_double((double)x[b2] + (double)i * d0 + 0.5D);
                aint3[b3] = MathHelper.floor_double((double)x[b3] + (double)i * d1 + 0.5D);
                byte b5 = 0;
