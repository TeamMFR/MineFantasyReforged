...
public class BombDispenser implements IBehaviorDispenseItem
{
	...
	public ItemStack dispense(IBlockSource dispenser, ItemStack item)
	{
		...
		if(item.getItem() instanceof ItemBomb)
		{
			...
			double posX = dispenser.getX() + ((float)direction.getFrontOffsetX()/2F);
			double posY = dispenser.getY() + ((float)direction.getFrontOffsetY()/2F);
			double posZ = dispenser.getZ() + ((float)direction.getFrontOffsetZ()/2F);
			...
			if (!world.isRemote)
	        {
	        	EntityBomb bomb = new EntityBomb(world).setType(ItemBomb.getFilling(item), ItemBomb.getCasing(item), ItemBomb.getFuse(item), ItemBomb.getPowder(item));
