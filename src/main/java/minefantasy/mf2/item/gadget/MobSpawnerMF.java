...
public class MobSpawnerMF extends ItemComponentMF
{
    ...
    {
		list.add(StatCollector.translateToLocal("item.spawn_"+types[Math.min(types.length-1, item.getItemDamage())]+".desc"));
    }	
	@Override
    ...
    {
		return StatCollector.translateToLocal("item.spawn_"+types[Math.min(types.length-1, item.getItemDamage())]+".name");
    }
	
	@Override
	public boolean onItemUse(ItemStack item, EntityPlayer user, World world, int blockX, int blockY, int blockZ, int face, float ox, float oy, float oz)
    {
        if (world.isRemote)
        {
            return true;
        ...
        else
        {
            ...
            Entity entity = spawnCreature(world, item.getItemDamage(), (double)blockX + 0.5D, (double)blockY + d0, (double)blockZ + 0.5D);
	...
	private EntityLivingBase getLiving(World world, int id) 
	{
		switch(id)
		{
		case 0: return new EntityDragon(world);
		case 1: return new EntityMinotaur(world);
		case 2: return new EntityHound(world);
		case 3: return new EntityCogwork(world);
		
			default: return new EntityDragon(world);
		}
	}
	@Override
	@SideOnly(Side.CLIENT)
