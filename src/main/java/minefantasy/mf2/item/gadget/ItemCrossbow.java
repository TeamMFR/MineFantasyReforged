...
public class ItemCrossbow extends Item implements IFirearm, IDisplayMFAmmo, IDamageModifier, IDamageType, IScope, ISpecialSalvage
{
    ...
    public void onUsingTick(ItemStack item, EntityPlayer player, int time)
    {
    	...
    	if(time == (max-5) && getUseAction(item).equalsIgnoreCase("reload") && (loaded == null || loaded.stackSize < getAmmoCapacity(item)))
    	{
    ...
    public void onPlayerStoppedUsing(ItemStack item, World world, EntityPlayer user, int time)
    {
    	...
    	if(action.equalsIgnoreCase("fire") && this.onFireArrow(user.worldObj, AmmoMechanicsMF.getArrowOnBow(item), item, user, this.getFullValue(item, "power"), false))
		{
			if(loaded != null)
			{
				--loaded.stackSize;
    ...
    public void addInformation(ItemStack item, EntityPlayer user, List list, boolean fullInfo)
    {
    	...
    	list.add(StatCollector.translateToLocalFormatted("attribute.crossbow.recoil.name", getFullValue(item, "recoil")));
    	list.add(StatCollector.translateToLocalFormatted("attribute.crossbow.spread.name", getFullValue(item, "spread")));
	...
	public boolean onFireArrow(World world, ItemStack arrow, ItemStack bow, EntityPlayer user, float charge, boolean infinite) 
	{
        ...
        EntityArrowMF entArrow = ammo.getFiredArrow(new EntityArrowMF(world, user, getFullValue(bow, "spread"), charge*2.0F), arrow);
	...
	{
		return getUseAction(item).equalsIgnoreCase("fire") ? getModifierForPart(item, "mod", "zoom") : 0F;//only mod affects zoom;
	}
