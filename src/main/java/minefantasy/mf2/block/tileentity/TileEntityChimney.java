...
public class TileEntityChimney extends TileEntity implements ISmokeCarrier
{
	...
	private boolean tryShareSmoke() 
	{
		...
		if(tryPassTo(-1, 0, 0, false, true) || tryPassTo(1, 0, 0, false, true) || tryPassTo(0, 0, -1, false, true) || tryPassTo(0, 0, 1, false, true))
		{
			return true;//Sides
		}
	...
	private boolean tryPassTo(int x, int y, int z, boolean priority, boolean sideways)
	{
		...
		if(tile != null && tile instanceof TileEntityChimney)
		{
			...
			int limit = priority ? max : this.getSmokeValue();//When going up, it fills all, sideways follows concentration gradient
