...
public class TileEntityBombBench extends TileEntity implements IInventory, ISidedInventory, IBasicMetre
{
	...
	{
		boolean sticky = !pressUsed && ResearchLogic.hasInfoUnlocked(user, KnowledgeListMF.stickybomb) && user.getHeldItem() != null && user.getHeldItem().getItem() == Items.slime_ball;
		...
		else if((pressUsed || ToolHelper.getCrafterTool(user.getHeldItem()).equalsIgnoreCase("spanner")))
		{
			...
			{
				worldObj.playSoundEffect(xCoord+0.5, yCoord, zCoord+0.5, "minefantasy2:block.twistbolt", 0.25F, 1.0F);
	...
	public void syncData()
	{
		...
		for(int i = 0; i < players.size(); i++)
		{
			...
			((WorldServer)worldObj).getEntityTracker().func_151248_b(player, new BombBenchPacket(this).generatePacket());
		}
	}
