	...
	private void craftItem(EntityPlayer lastHit) {
		if (this.canCraft()) {
			...
			if (hot) {
				...
				if (inventory[output] == null || (inventory[output].stackSize == 1
						&& SpecialForging.getItemDesign(inventory[output]) != null)) {
					addXP(lastHit);
					this.inventory[output] = out;
					consumeResources();
				} else if (inventory[output].isItemEqual(out)
						&& (inventory[output].stackSize + out.stackSize <= getStackSize(inventory[output]))) {
					addXP(lastHit);
				...
				if (this.inventory[output] == null) {
					...
					this.inventory[output].stackSize += result.stackSize; // Forge BugFix: Results may have multiple
																			// items
				}
				addXP(lastHit);
				consumeResources();
