...
public class BlockCogwork extends BlockDirectional
{
    ...
    {
        return side == 1 ? this.toptex : (side == 0 ? this.toptex : (meta == 2 && side == 2 ? this.facetex : (meta == 3 && side == 5 ? this.facetex : (meta == 0 && side == 3 ? this.facetex : (meta == 1 && side == 4 ? this.facetex : this.blockIcon)))));
    }
    ...
    {
    	if (isMain && PowerArmour.isStationBlock(world, x, y+2, z) && world.getBlock(x, y - 1, z) == BlockListMF.cogwork_legs && world.getBlock(x, y + 1, z) == BlockListMF.cogwork_helm)
        {
            if (!world.isRemote)
            {
                ...
                suit.setLocationAndAngles((double)x + 0.5D, (double)y - 0.95D, (double)z + 0.5D, angle, 0.0F);
	...
	{
		int l = MathHelper.floor_double((double)(user.rotationYaw * 4.0F / 360.0F) + 2.5D) & 3;
		return (int)(l*90);
	}
