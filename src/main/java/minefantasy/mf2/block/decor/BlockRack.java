...
public class BlockRack extends BlockWoodDecor 
{
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer user, int i, float f, float f1, float f2) 
    {
    	TileEntityRack tile = (TileEntityRack)world.getTileEntity(x, y, z);
		if(world.isRemote)
 		{
			...
			if(slot >= 0 && slot < 4)
			{
				((EntityClientPlayerMP)user).sendQueue.addToSendQueue(new RackCommand(slot, user, tile).generatePacket());
			}
 		}
