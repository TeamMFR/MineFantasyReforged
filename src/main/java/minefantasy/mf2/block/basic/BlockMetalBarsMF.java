...
public class BlockMetalBarsMF extends BlockPane 
{
	...
	{
		super("minefantasy2:metal/"+material.name.toLowerCase()+"_bars", "minefantasy2:metal/"+material.name.toLowerCase()+"_bars", Material.iron, true);
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity hitter)
    {
		if(baseMat.name.equalsIgnoreCase("dragonforge") && !hitter.isImmuneToFire() && hitter instanceof EntityLivingBase)
		{
			...
			world.spawnParticle("smoke", x+rand.nextDouble(), y+rand.nextDouble(), z+rand.nextDouble(), 0D, 0D, 0D);
		}
    }
}
