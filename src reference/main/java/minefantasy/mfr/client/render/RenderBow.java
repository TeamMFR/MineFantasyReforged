...
public class RenderBow implements IItemRenderer {
    ...
    public static void renderEnchantmentEffects(Tessellator tessellator) {
        ...
        FMLClientHandler.instance().getClient().renderEngine.bindTexture(TextureHelperMFR.ITEM_GLINT);
    ...
    private void renderEquippedBow(ItemStack item, EntityLivingBase entityLivingBase, boolean firstPerson) {
        ...
        ItemBowMFR bow = null;
        if (item != null && item.getItem() instanceof ItemBowMFR) {
            bow = (ItemBowMFR) item.getItem();
        }

        ItemStack arrowStack = AmmoMechanicsMFR.getArrowOnBow(item);
