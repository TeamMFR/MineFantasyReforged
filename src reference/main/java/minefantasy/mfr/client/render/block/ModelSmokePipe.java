...
public class ModelSmokePipe extends ModelBase {
    ...
    public void renderModel(TileEntityChimney tile, float f) {
        ...
        if (tile == null) {
            ...
            if (tile.canAccept(EnumFacing.UP))
                up.render(f);
            if (tile.canAccept(EnumFacing.DOWN))
                down.render(f);
            if (tile.canAccept(EnumFacing.NORTH))
                north.render(f);
            if (tile.canAccept(EnumFacing.SOUTH))
                south.render(f);
            if (tile.canAccept(EnumFacing.EAST))
                east.render(f);
            if (tile.canAccept(EnumFacing.WEST))
                west.render(f);
        }
    }
}