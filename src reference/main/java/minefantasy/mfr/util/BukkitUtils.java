...
public final class BukkitUtils {
    ...
    public static final boolean cantBreakBlock(EntityPlayer player, BlockPos pos) {
        try {
            EntityPlayer bukkitPlayer = toBukkitEntity(player);
            BlockBreakEvent event = new BlockBreakEvent(bukkitPlayer.getWorld().getBlockAt(x, y, z), bukkitPlayer);
            callEvent(event);
            return event.isCancelled();
        } catch (Throwable throwable) {
            MFRLogUtil.logWarn(String.format("Failed call BlockBreakEvent: [Player: %s, X:%d, Y:%d, Z:%d]",
