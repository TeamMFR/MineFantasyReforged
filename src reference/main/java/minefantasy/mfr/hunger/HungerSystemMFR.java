package minefantasy.mfr.hunger;
...
public class HungerSystemMFR {
    ...
    public void slowHunger(EntityPlayer player) {
        ...
        if (food < prevFood) {
            ...
            MineFantasyRebornAPI.debugMsg("Saturation: " + sat + " Temp: " + temp);

            if (slowdownRate < 0 || temp > 0) {
                MineFantasyRebornAPI.debugMsg("Hunger drop cancelled: Temp");
                setTempFood(player, temp - 1);
                player.getFoodStats().addStats(1, 0.0F);
            } else if (sat > 0) {
                decrSaturation(player);
                MineFantasyRebornAPI.debugMsg("Hunger drop cancelled: Sat = " + (sat - 1));
                player.getFoodStats().addStats(1, 0.0F);
            }
